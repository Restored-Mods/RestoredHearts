...
local function load()
    ...
    function ComplianceImmortal.HealImmortalHeart(player) -- returns true if successful
        if ComplianceImmortal.GetImmortalHeartsNum(player) > 0 and ComplianceImmortal.GetImmortalHeartsNum(player) % 2 ~= 0 then
            local ImmortalEffect = Isaac.Spawn(EntityType.ENTITY_EFFECT, RestoredHearts.Enums.Entities.IMMORTAL_HEART_CHARGE.Variant, 0, player.Position + Vector(0, 1), Vector.Zero, nil):ToEffect()
            ImmortalEffect:GetSprite().Offset = Vector(0, -22)
            SFXManager():Play(RestoredHearts.Enums.SFX.Hearts.IMMORTAL_PICKUP, 1, 0)
