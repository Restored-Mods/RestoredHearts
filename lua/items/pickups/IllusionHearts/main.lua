...
local function InstaDeath(p)
	if TSIL.SaveManager.GetPersistentVariable(RestoredHearts, "IllusionInstaDeath") == 2 then
	...
	IllusionMod.KillIllusion(p, TSIL.SaveManager.GetPersistentVariable(RestoredHearts, "IllusionInstaDeath") == 2)
end
...
function IllusionModLocal:UpdateClones(p)
	...
	if data.IsIllusion then
		...
		if p:IsDead()  then
			...
			if p:GetSprite():IsFinished() and p:GetSprite():GetAnimation():match("Death") == "Death" or TSIL.SaveManager.GetPersistentVariable(RestoredHearts, "IllusionInstaDeath") == 2 then
				p:GetSprite():SetLastFrame()
				if TSIL.SaveManager.GetPersistentVariable(RestoredHearts, "IllusionInstaDeath") == 2 then
...
RestoredHearts:AddCallback(ModCallbacks.MC_POST_PLAYER_UPDATE, IllusionModLocal.UpdateClones)
...
RestoredHearts:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, IllusionModLocal.CloneRoomUpdate)
...
RestoredHearts:AddCallback(ModCallbacks.MC_POST_PEFFECT_UPDATE, IllusionModLocal.CloneColor)
...
RestoredHearts:AddCallback(ModCallbacks.MC_EVALUATE_CACHE, IllusionModLocal.Cache)
...
RestoredHearts:AddCallback(ModCallbacks.MC_PRE_PLAYER_COLLISION, IllusionModLocal.preIllusionWhiteFlame)

---@param pickup EntityPickup
function IllusionModLocal:PreGoldenSpawn(pickup)
	if TSIL.Random.GetRandom(pickup.InitSeed) >= (1 - TSIL.SaveManager.GetPersistentVariable(RestoredHearts, "IllusionHeartSpawnChance") / 100) 
...
RestoredHearts:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, IllusionModLocal.onEntityTakeDamage, EntityType.ENTITY_PLAYER)
...
RestoredHearts:AddCallback(ModCallbacks.MC_POST_ENTITY_REMOVE, IllusionModLocal.AfterDeath)
...
RestoredHearts:AddCallback(ModCallbacks.MC_POST_NPC_RENDER, IllusionModLocal.DarkEsau, EntityType.ENTITY_DARK_ESAU)
...
RestoredHearts:AddCallback(ModCallbacks.MC_POST_FAMILIAR_RENDER, IllusionModLocal.Familiar)
...
RestoredHearts:AddCallback(ModCallbacks.MC_POST_KNIFE_RENDER, IllusionModLocal.Knife)

function IllusionModLocal:ClonesControls(entity,hook,action)
	if entity ~= nil and entity.Type == EntityType.ENTITY_PLAYER and TSIL.SaveManager.GetPersistentVariable(RestoredHearts, "IllusionClonesPlaceBombs") == 1 then
...
RestoredHearts:AddCallback(ModCallbacks.MC_INPUT_ACTION, IllusionModLocal.ClonesControls)