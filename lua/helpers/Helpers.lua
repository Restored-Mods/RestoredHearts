...
function Helpers.ChargeBowl(player)
	for slot = 0,2 do
		if player:GetActiveItem(slot) == RestoredHearts.Enums.CollectibleType.COLLECTIBLE_BOWL_OF_TEARS then
			local charge = Helpers.GetCharge(player,slot)
...
function Helpers.TurnEnemyIntoGoldenMachine(enemy, player, rng)
    ...
    for _, luckySevenSlot in ipairs(RestoredHearts.LuckySevenSpecialSlots) do
        if luckySevenSlot:CanSpawn(player) then
            machinesToUse[#machinesToUse+1] = luckySevenSlot
        end
    end

    local chosenMachine = machinesToUse[rng:RandomInt(#machinesToUse)+1] or RestoredHearts.LuckySevenRegularSlot

    local luckySevenSlotEntity = Isaac.Spawn(EntityType.ENTITY_SLOT, RestoredHearts.Enums.Entities.LUCKY_SEVEN_SLOT.Variant, 0, enemy.Position, Vector.Zero, nil)
...
function Helpers.scheduleForUpdate(foo, delay, callback)
	if REPENTOGON then
		...
		if not delayedFuncs[callback] then
			...
			RestoredHearts:AddCallback(callback, function()
				runUpdates(delayedFuncs[callback])
			end)
			local function Reset()
				delayedFuncs[callback] = {}
			end
			RestoredHearts:AddCallback(ModCallbacks.MC_POST_GAME_STARTED, Reset)
			RestoredHearts:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, Reset)
		end
